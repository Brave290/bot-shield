  ...
  return (<>
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#020617] pt-20 lg:flex lg:overflow-visible">
      <aside className="fixed bottom-0 left-0 right-0 z-50 h-[76px] border-t border-slate-800 bg-[#07101e]/95 px-2 py-2 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl lg:sticky lg:top-20 lg:bottom-auto lg:h-[calc(100vh-5rem)] lg:w-72 lg:shrink-0 lg:border-r lg:border-t-0 lg:bg-slate-950/60 lg:px-4 lg:py-6">
        ...
        <nav className="flex h-full items-stretch justify-around gap-1 lg:mt-6 lg:block lg:h-auto lg:space-y-7">
          <div className="flex w-full items-stretch lg:block lg:space-y-1"><p className="mb-2 hidden px-3 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-600 lg:block">Workspace</p>{[["overview", "Overview", "chart"], ["users", "Users", "users"], ["rules", "Project rules", "shield"], ["pricing", "Pricing", "file"]].map(([id, label, icon]) => <button key={id} onClick={() => changeTab(id as Tab)} className={`flex min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-xl px-1 py-1 text-[10px] font-medium transition lg:w-full lg:flex-row lg:justify-start lg:gap-3 lg:px-3 lg:py-3 lg:text-sm ${tab === id ? "bg-blue-600/15 text-blue-300 ring-1 ring-blue-500/30" : "text-slate-500 hover:bg-slate-900 hover:text-white"}`}><Icon name={icon} className="h-4 w-4" /><span className="max-w-full truncate">{label}</span></button>)}</div>
