    ...
    <DashboardShell userType="user" userName={user?.email} onLogout={handleLogout}>
      <div className="space-y-6 sm:space-y-8">
        ...
        {tab === "projects" && (
          ...
          <section className="mb-6 rounded-2xl border border-blue-500/20 bg-blue-500/5 p-5" aria-labelledby="setup-health-title">
            <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-400">Setup health</p><h2 id="setup-health-title" className="mt-2 text-xl font-semibold text-white">Get protected in four steps</h2><p className="mt-1 text-sm text-slate-400">Finish the essentials before sending production traffic through BotShield.</p></div><span className="rounded-full bg-blue-500/10 px-3 py-1 text-xs text-blue-300">{Math.min(4, (projects.length ? 1 : 0) + (projects[0]?.domain ? 1 : 0) + (firstProjectKey ? 1 : 0) + (usage?.month ? 1 : 0))}/4 complete</span></div>
            <div className="mt-5 grid gap-2 sm:grid-cols-4">{[[Boolean(projects.length), "Create a project", "/dashboard/new-project"], [Boolean(projects[0]?.domain), "Add your domain", projects[0] ? `/dashboard/${projects[0].id}` : "/dashboard"], [Boolean(firstProjectKey), "Install the SDK", "/dashboard?tab=sdk"], [Boolean(usage?.month), "Run first verification", "/test"]].map(([done, label, href]) => <button key={String(label)} onClick={() => router.push(String(href))} className={`rounded-xl border px-3 py-3 text-left text-xs transition ${done ? "border-emerald-500/30 bg-emerald-500/5 text-emerald-300" : "border-slate-800 bg-slate-950/50 text-slate-300 hover:border-blue-500/50"}`}><span className="mr-2">{done ? "✓" : "○"}</span>{label}</button>)}</div>
          </section>
          {projects.length === 0 ? (
